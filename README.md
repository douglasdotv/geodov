...
A userscript captures guesses and their respective locations during gameplay, storing the relevant information in a database. This data is then displayed in a [web application](https://geodov.com), which provides a detailed history of each round (showing the game type, the guess, the actual location, the distance, and the time taken), as well as a map with all visited locations and some statistics by country.
